...
def calculate_accuracy(user_answers, correct_answers):
    ...
    print(f"Correct Count: {correct_count}, Total Count: {total_count}, Accuracy: {accuracy}")

    return accuracy

@app.route('/result')
def result():
    candidate_key = request.args.get('candidate_key')
    conn = sqlite3.connect('chatbot.db')
    cursor = conn.cursor()
    cursor.execute('''
        SELECT accuracy FROM correct_answers WHERE candidate_key = ? 
    ''', (candidate_key,))                           
    accuracy = cursor.fetchone()
    conn.close()
    
    accuracy = accuracy[0] if accuracy else 0
    print(f"Fetching result for candidate key: {candidate_key}")
    print(f"Result accuracy retrieved: {accuracy}")
